...
function setNameInputActions() {
  for(var i = 0; i < names.length; i++) {
    document.getElementById(names[i]).addEventListener('change', () => {
      saveData(players[i], document.getElementById(names[i]).value);
    });
  }
...
function setRoleInputActions() {
  for(var i = 0; i < roles.length; i++) {
    document.getElementById(roles_ids[i]).addEventListener('change', () => {
      saveData(roles[i], document.getElementById(roles_ids[i]).value);
      removeUniqueRoles();
    });
  }
...
function removeUniqueRoles() {
  ...
  // uniqueCheck(['town-support', 'town-random'], "mayor", "Mayor");
  // uniqueCheck(['town-support', 'town-random'], "retri", "Retributionist");
}
